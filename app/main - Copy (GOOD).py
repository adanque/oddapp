...
#     message = Mail(
#         from_email='adanque@cen-gen.com',  # Replace with your verified sender email
#         to_emails=to_email,
#         subject=subject,
#         plain_text_content=content
#     )
...
async def process_messages():
    ...
    # await receiver.__aenter__()  # Manually enter the receiver context
    ...
    #             email_data = json.loads(str(msg))
    #             tasks.append(send_email(email_data['to_email'], email_data['subject'], email_data['content']))
    #             await receiver.complete_message(msg)  # Mark the message as processed

    #         if tasks:
    #             await asyncio.gather(*tasks)  # Send all emails concurrently

    #         await asyncio.sleep(1)  # Optional: Wait before checking for new messages again
    # finally:
    #     await receiver.__aexit__(None, None, None)  # Ensure the receiver is properly exited

# Route to serve the web input form
...
#     content: str = Form(...)
# ):
#     # Prepare the message and send it to the Service Bus queue
#     email_data = {
#         'to_email': to_email,
#         'subject': subject,
#         'content': content
...
#     await sender.__aenter__()  # Manually enter the sender context
    
#     try:
#         message = ServiceBusMessage(json.dumps(email_data))
#         await sender.send_messages(message)  # Send the message to the queue
#     finally:
#         await sender.__aexit__(None, None, None)  # Ensure the sender is properly exited

#     return {"message": "Email request submitted!"}

# # Startup event to begin processing messages from the Service Bus

# @app.repeat_every(seconds=60*30)
...
# async def long_running_task_with_loop():
#     for i in range(5):
#         print(f"Long running task iteration {i}")
#         asyncio.create_task(process_messages())
#         await asyncio.sleep(1)
#     print("Long running task finished.")

# @app.post("/start_background_task")
# async def start_background_task(background_tasks: BackgroundTasks):
#     background_tasks.add_task(long_running_task_with_loop)
#     return {"message": "Background task started!"}    

# @repeat_every(seconds=5)  # Run every 5 seconds
# async def do_something_repeatedly():
#     print("Running a repeated task!")
#     # Your asynchronous or synchronous task logic here
#     asyncio.create_task(process_messages())
...
# # Asynchronous function to send an email using SendGrid
# async def send_email(to_email: str, subject: str, content: str):
#     message = Mail(
#         from_email='adanque@cen-gen.com',  # Replace with your verified sender email
#         to_emails=to_email,
...
# # Asynchronous function to process messages from the Service Bus
# async def process_messages():
